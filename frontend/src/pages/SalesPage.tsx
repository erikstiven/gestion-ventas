    ...
    <>
      <Card>
        <CardHeader>
          ...
          <div className="grid gap-2 sm:grid-cols-6 items-center">
            <div className="relative sm:col-span-2">
              ...
              <Input className="pl-8" placeholder="Buscar cliente/vendedor" value={filters.search} onChange={e=>setFilters(f=>({...f,search:e.target.value}))}/>
            </div>
            ...
            <Input type="number" placeholder="Año" value={filters.year} onChange={e=>setFilters(f=>({...f,year:e.target.value}))}/>
            <Button variant="outline" onClick={()=>fetch(filters)}>Filtrar</Button>
        ...
        <CardContent>
          ...
          <Table>
            <THead><Th>ID</Th><Th>Fecha</Th><Th>Total</Th><Th>Cliente</Th><Th>Vendedor</Th><Th>Zona</Th><Th children={undefined}/></THead>
            <TBody>
              {items.map((r:Sale)=>(
                <tr key={r.id} className="border-t">
                  ...
                  <Td>{r.zona?.nombre}</Td>
                  <Td><div className="flex justify-end"><Button variant="danger" onClick={()=>setToDelete(r.id)}><Trash2 className="h-4 w-4"/></Button></div></Td>
                </tr>
              ))}
            </TBody>
          </Table>
          <Pagination page={page} perPage={per_page} total={total} onPage={(p)=>fetch({...filters, page:p})}/>
        </CardContent>
      </Card>

      <Modal open={open} onOpenChange={()=>setOpen(false)} title="Nueva venta">
