...
export default function SaleForm({ onDone }: { onDone: () => void }) {
  ...
  // Mantener items sincronizados con RHF
  useEffect(() => {
    setValue("detalles", items as any, { shouldValidate: true });
  }, [items, setValue]);
  ...
  const addItem = () => {
    ...
    setItems((prev) => [
      ...prev,
      { product_id: p?.id ?? 0, cantidad: 1, precio_unitario: p?.precio ?? 0 },
    ]);
  };
  ...
  const total = useMemo(
    () => items.reduce((s, it) => s + it.cantidad * it.precio_unitario, 0),
    [items]
  );
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
                          ...
                          prev.map((x, i) =>
                            i === idx
                              ? {
                                  ...x,
                                  product_id: id,
                                  precio_unitario: p?.precio ?? x.precio_unitario,
                                }
                              : x
                          )
                        );
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setItems((prev) =>
                          prev.map((x, i) =>
                            i === idx
                              ? { ...x, precio_unitario: Number(e.target.value) }
                              : x
                          )
                        )
                      }
      ...
      {errors.detalles && (
        <p className="text-xs text-red-600">{String(errors.detalles.message)}</p>
      )}
